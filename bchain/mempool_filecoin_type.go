...
type MempoolFilecoinType struct {
	BaseMempool
...
func NewMempoolFilecoinType(chain BlockChain, mempoolTxTimeoutHours int, queryBackendOnResync bool) *MempoolFilecoinType {
	mempoolTimeoutTime := time.Duration(mempoolTxTimeoutHours) * time.Hour
	return &MempoolFilecoinType{
...
func (m *MempoolFilecoinType) createTxEntry(txid string, txTime uint32) (txEntry, bool) {
...
func (m *MempoolFilecoinType) Resync() (int, error) {
...
func (m *MempoolFilecoinType) AddTransactionToMempool(txid string) {
	m.mux.Lock()
...
func (m *MempoolFilecoinType) RemoveTransactionFromMempool(txid string) {
	m.mux.Lock()
